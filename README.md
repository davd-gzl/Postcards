...
- **Travel log** — record journeys you've actually taken (flight, train, bus, ferry, car) between two places; each trip's **great-circle distance** is derived from the endpoints' coordinates, trips are drawn as **arcs on the map**, and totals roll up trips + kilometres + a per-mode breakdown. **Filter the log by year and month** — the list and totals narrow to any period. **Import a flight from a boarding pass** — scan its barcode or paste the code, and the IATA **BCBP** is decoded **on-device** into a trip. A log of past travel, not a planner.
...
- **Time filter for the travel log** — narrow the trip list *and* its totals to a chosen **year** (and month); a per-year rollup of "trips, kilometres, by mode" falls out for free.
