# Your published Postcards journey
...
> This is the exact text shipped inside each export (see `hosting.ts` /
> `HOSTING_README`). The repo-level guide is `docs/publishing.md`.
...
- **GitHub Pages** — put `index.html` in a repository, then in _Settings →
  Pages_ choose the branch and `/ (root)` folder. Your site appears at
  `https://<user>.github.io/<repo>/`.
- **Netlify / Cloudflare Pages / Vercel** — drag the folder onto their "deploy"
  drop zone. No configuration needed.
- **Nextcloud / ownCloud** — upload `index.html`, then use _Share → Share link_.
...
The published site collects **nothing**. If you self-host and _want_ analytics,
